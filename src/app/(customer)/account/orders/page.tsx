...
function OrdersInner() {
  ...
  const handleReturnRequest = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!token || !returnItemId) return;

    setSubmittingReturn(true);
    try {
      const res = await fetch(`/api/orders/return/${returnItemId}?reason=${encodeURIComponent(returnReason)}`, {
        method: 'POST',
  ...
  return (
    <div className="space-y-4">
      {orders.map((order) => (
        <div key={order.id} className="card overflow-hidden shadow-sm hover:shadow-md transition-shadow duration-200">
          <div className="p-4 border-b flex flex-wrap items-center justify-between gap-2 bg-gray-50/50">
            <div className="flex items-center gap-3">
              <span className="font-mono font-bold text-gray-900">{order.orderId}</span>
              <span className={`text-[10px] uppercase font-bold px-2 py-0.5 rounded-full ${statusColor[order.status] || 'bg-gray-100'}`}>
                {order.status}
              </span>
            </div>
          ...
          <div className="px-4 py-2 bg-white flex flex-col gap-1 border-b">
            <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest">
              <div className={`w-2 h-2 rounded-full ${order.status === 'placed' ? 'bg-yellow-400' : 'bg-gray-200'}`}></div>
              <span className={order.status === 'placed' ? 'text-gray-900' : 'text-gray-400'}>Ordered</span>
              <div className="flex-1 border-t border-gray-100"></div>
              <div className={`w-2 h-2 rounded-full ${order.status === 'shipped' ? 'bg-blue-400' : 'bg-gray-200'}`}></div>
              <span className={order.status === 'shipped' ? 'text-gray-900' : 'text-gray-400'}>Shipped</span>
              <div className="flex-1 border-t border-gray-100"></div>
              <div className={`w-2 h-2 rounded-full ${order.status === 'delivered' ? 'bg-green-400' : 'bg-gray-200'}`}></div>
              <span className={order.status === 'delivered' ? 'text-gray-900' : 'text-gray-400'}>Delivered</span>
            </div>
                  ...
                  {order.status === 'delivered' && !item.returnStatus && !item.replacementStatus && (
                    <div className="flex gap-2">
                      {item.product?.isReturnable && (
                        <button
                          onClick={() => { setReturnItemId(item.id); setReturnReason('Damaged Product'); }}
                          className="text-[10px] text-accent font-bold uppercase border border-accent px-2 py-0.5 rounded hover:bg-accent hover:text-white transition-colors"
                        >
                          Return
                        </button>
                      )}
          ...
          <div className="p-4 bg-gray-50/30 flex justify-between items-center border-t border-gray-100">
            <div className="flex flex-col">
              <span className="text-xs text-gray-400">Payment Status</span>
              <span className="text-sm font-medium text-gray-700 capitalize">{order.paymentStatus}</span>
            </div>
            <div className="flex flex-col items-end">
              <span className="text-xs text-gray-400">Total Amount</span>
              <span className="text-lg font-bold text-accent">{formatPrice(order.total)}</span>
            </div>
          </div>
        </div>
      ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-2xl animate-in fade-in zoom-in duration-200">
            ...
            <form onSubmit={handleReturnRequest} className="space-y-4">
              <div>
                <label className="text-xs text-gray-500 block mb-1">Reason for Return</label>
                <select
                  value={returnReason}
                ...
                >
                  ...
                  <option value="Not Liked by Customer">Not Liked by Customer</option>
          ...
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-2xl animate-in fade-in zoom-in duration-200">
            ...
            <form onSubmit={handleReplacementRequest} className="space-y-4">
              ...
              <div className="flex gap-2">
                ...
                <button
                  type="submit"
                  className="flex-1 btn-primary py-2 rounded-lg text-sm bg-blue-600 border-blue-600 hover:bg-blue-700 hover:border-blue-700"
                  disabled={submittingReplacement}
                >

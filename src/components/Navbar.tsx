  ...
  return (
    <nav className="sticky top-0 z-50 glass">
      <div className="max-w-6xl mx-auto px-4 flex items-center justify-between h-16">
        ...
        <Link href="/admin" className="text-gray-500 text-xs font-bold uppercase tracking-widest hover:text-accent transition-colors hidden sm:block">
          Admin
        </Link>
          ...
          {user ? (
            <div className="relative group">
              ...
              <div className="absolute right-0 mt-2 p-1 w-48 bg-white/80 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/20 opacity-0 scale-95 invisible group-hover:opacity-100 group-hover:visible group-hover:scale-100 transition-all duration-300 origin-top-right">
                ...
                <button onClick={logout} className="flex items-center gap-3 w-full text-left px-4 py-3 text-sm font-black text-red-500 hover:bg-red-50 rounded-xl transition-colors">
                  LOGOUT
                </button>
              </div>
            </div>
